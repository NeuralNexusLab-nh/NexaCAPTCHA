(function () {
  ...
  var translations = {
    en: {
      ...
      featureIncompleteTitle: "Incomplete by design", featureIncompleteBody: "Most frames show only fragments. At least one character is never shown in full.", featureDistortionTitle: "Constant distortion", featureDistortionBody: "Characters keep bending, stretching, rotating, and changing shape.", featureMotionTitle: "Independent movement", featureMotionBody: "Every character moves in its own direction and at its own changing speed.", featureWindowTitle: "An unpredictable window", featureWindowBody: "The visible area speeds up, slows down, and moves backward without a simple rhythm.", featureUniqueTitle: "Different every time", featureUniqueBody: "The motion, distortion, timing, and hidden character change with every CAPTCHA.", featureCostTitle: "Higher solving cost", featureCostBody: "Bots must inspect many frames, track moving fragments, and rebuild the answer instead of reading one image.",
    ...
    "zh-Hant": {
      ...
      featureIncompleteTitle: "刻意保持不完整", featureIncompleteBody: "大部分畫面只顯示片段，而且至少一個字元從頭到尾都不會完整出現。", featureDistortionTitle: "持續扭曲", featureDistortionBody: "字元會不斷彎曲、拉伸、旋轉和改變形狀。", featureMotionTitle: "各自獨立移動", featureMotionBody: "每個字元都有自己的方向和持續改變的速度。", featureWindowTitle: "難以預測的窗口", featureWindowBody: "可見範圍會加速、減速和倒退，沒有簡單規律。", featureUniqueTitle: "每次都不一樣", featureUniqueBody: "每次 CAPTCHA 的移動、扭曲、時機和隱藏字元都會改變。", featureCostTitle: "大幅提高破解成本", featureCostBody: "Bot 不能只讀一張圖片，而必須檢查許多畫面、追蹤片段並重組答案。",
    ...
    ja: {
      ...
      featureIncompleteTitle: "意図的に不完全", featureIncompleteBody: "ほとんどの画面は断片だけで、少なくとも1文字は最後まで完全に表示されません。", featureDistortionTitle: "変化し続ける歪み", featureDistortionBody: "文字は曲がり、伸び、回転し、形を変え続けます。", featureMotionTitle: "独立した動き", featureMotionBody: "各文字は独自の方向と変化する速度で動きます。", featureWindowTitle: "予測しにくい表示窓", featureWindowBody: "表示範囲は加速、減速、後退し、単純な規則がありません。", featureUniqueTitle: "毎回異なる", featureUniqueBody: "動き、歪み、タイミング、隠される文字が CAPTCHA ごとに変わります。", featureCostTitle: "解読コストを大幅に増加", featureCostBody: "Bot は一枚の画像では足りず、多数の画面を調べ、断片を追跡して答えを再構成します。",
